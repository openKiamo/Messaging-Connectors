...
class MessagingManager
{
  public    function __construct( $_parent )
  {
    ...
    $this->log( "Service : " . $this->getConf( 'self', 'service' ), Logger::LOG_DEBUG, __METHOD__ ) ;
    $this->log( "Version : " . $this->getConf( 'self', 'version' ), Logger::LOG_DEBUG, __METHOD__ ) ;
  ...
  */
  ...
  public   function _buildOAuthBaseParams()
  {
    $res = [
      ...
      $this->getConf( 'access', 'oauthData.keys.nonce'           ) => Uuids::get( true, 32 ),
      $this->getConf( 'access', 'oauthData.keys.signatureMethod' ) => $this->getConf( 'access', 'oauthData.signatureMethod' ),
      $this->getConf( 'access', 'oauthData.keys.timestamp'       ) => Datetimes::nowTs(),
...
class Resources
{
  ...
  {
    if( file_exists( Resources::DATA_PATH ) === true ) return ;
    mkdir( Resources::DATA_PATH ) ;
  }

  public static function getResourcePath( $name )
  {
    return Resources::getDataFolderPath( 1 ) . Resources::_getFileName( $name ) ;
  }

  public static function srm( $name )
  {
    $filepath = Resources::getResourcePath( $name ) ;
    if( file_exists( $filepath ) !== true ) return ;
    unlink( $filepath ) ;
  }

  public static function fileInfos( $name )
  {
    return pathinfo( Resources::getResourcePath( $name ) ) ;
  }
  ...
  private static function _getModuleName( $shift = 0 )
  {
    ...
    try { $modName = debug_backtrace( DEBUG_BACKTRACE_PROVIDE_OBJECT, 4 + $shift )[ 3 + $shift ][ 'object' ]->getClassName() ; } catch( \Exception $e ) {}
    return $modName ;
  }
  ...
  private  function defineTestFunctions()
  {
    ...
    $this->testFunctions[ 'test02' ] = [
      ...
      'function' => function()
      {
        ...
        //$lastReadMessageId = "xxxxxxxxxxxxxxx" ;
    ...
    $this->testFunctions[ 'test03' ] = [
      ...
      'function' => function()
      {
        ...
        $msg = 'Send message simple test #01' ;
